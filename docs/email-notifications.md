...
- When the scheduled check command is run. Site administrators should ask their developer to schedule `php craft verified-elements/check-expired-verifications` to run **once daily** (for example nightly via cron) so digests arrive predictably.
...
::: tip
The check has no deduplication — every run re-sends a full digest of everything currently expired. Scheduling the command more than once a day will email Reviewers duplicate digests for the same items, so once daily is the right interval.
:::

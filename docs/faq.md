...
> Draft. Extend this list with real support questions over time.
...
No expiry date is set. The element counts as Verified and will never expire. Elements in enabled sections that were never explicitly verified also show as Indefinite.
...
Support: <support@webhub.de>. Bug reports: the plugin's GitHub issues page.
